...
For the single-A100, sampled-token analogue of the Lightning-OPD standard
recipe, first build the pinned official DAPO-Math-17k parquet and then use the
dedicated launcher:
...
CONFIG_ONLY=1 bash opd_lightning_a100.sh   # resolve and verify Hydra config
SMOKE=1 bash opd_lightning_a100.sh         # three-step pipeline check
bash opd_lightning_a100.sh                 # full 150-step run
```

This launcher keeps the repository's Qwen3-1.7B-SFT/Qwen3-4B pair, so it is a
scaled reproduction rather than the paper's exact Qwen3-4B-SFT/Qwen3-8B run.
It assumes the SFT data was generated by the same Qwen3-4B teacher checkpoint.
